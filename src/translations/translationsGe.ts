export const translationsGe = {
  applicationRunning: 'აპლიკაცია მუშაობს!',

  // Authentication
  passwordLabel: 'პაროლი',
  passwordPlaceholder: 'შეიყვანეთ პაროლი',
  repeatPasswordLabel: 'გაიმეორეთ პაროლი',
  repeatPasswordPlaceholder: 'გაიმეორეთ პაროლი',
  rememberDevice: 'მოწყობილობის დამახსოვრება',
  emailLabel: 'ელ–ფოსტა',
  emailPlaceholder: 'შეიყვანეთ ელ–ფოსტა',

  // Login
  loginTitle: 'კეთილი იყოს თქვენი დაბრუნება',
  loginParagraph:
    'კეთილი იყოს თქვენი დაბრუნება! გთხოვთ შეიყვანოთ თქვენი ინფორმაცია',
  loginUsernameLabel: 'მომხმარებელს სახელი',
  loginUsernamePlaceholder: 'შეიყვანეთ მომხმარებლის სახელი ან ელ ფოსტა',
  forgotPassword: 'დაგავიწყდათ პაროლი?',
  loginButton: 'შესვლა',
  dontHaveAccount: 'არ გაქვთ ანგარიში?',
  signupFree: 'დარეგისტრირდი უფასოდ',

  // Signup
  signupTitle: 'კეთილი იყოს თქვენი მობრძანება კორონათაიმზე',
  signupParagraph: 'რეგისტრაციისთვის შეიყვანეთ მოთხოვნილი ინფორმაცია',
  signupUsernameLabel: 'მომხმარებლის სახელი',
  signupUsernamePlaceholder: 'შეიყვანეთ მომხმარებლის სახელი',
  signupUsernameNote:
    'მომხმარებლის სახელი უნდა იყოს უნიკალური, უნდა შედგებოდეს მინ 3 სიმბოლოსგან',
  signupButton: 'რეგისტრაცია',
  alreadyHaveAccount: 'გაქვთ უკვე ანგარიში?',
  login: 'შესვლა',

  // Reset
  resetPassword: 'პაროლის შეცვლა',
  newPasswordLabel: 'ახალი პაროლი',
  newPasswordPlaceholder: 'შეიყვანეთ ახალი პაროლი',
  saveChanges: 'ცვლილებების შენახვა',

  // Notification
  sendEmail: 'ჩვენ გამოგიგზავნეთ დამადასტურებელი ელ.წერილი',
  confirmAccount: 'თქვენი ანგარიში დადასტურებულია, შეგიძლიათ შეხვიდეთ',
  updatePassword: 'თქვენი პაროლი წარმატებით განახლდა',
  signin: 'შესვლა',

  // Header
  logout: 'გასვლა',

  // Landing Titles
  worldwideStatistics: 'მსოფლიო სტატისტიკა',
  statisticsByCountry: 'სტატისტიკა ქვეყნების მიხედვით',
  worldwide: 'მსოფლიო',
  byCountry: 'ქვეყნების მიხედვით',

  // Landing Headers
  newCases: 'ახალი შემთხვევები',
  recovered: 'გამოჯანმრთელებული',
  deaths: 'გარდაცვლილი',
  location: 'ლოკაცია',

  // By Country
  searchByCountry: 'მოძებნე ქვეყანა',

  // Hook Form Errors
  usernameRequired: 'მომხმარებლის სახელი აუცილებელია',
  usernameMinLength: 'მომხმარებლის სახელი უნდა შედგებოდეს მინ 3 სიმბოლოსგან',
  emailRequired: 'ელ ფოსტა აუცილებელია',
  emailValid: 'ელ ფოსტა უნდა იყოს ვალიდური',
  passwordRequired: 'პაროლი აუცილებელია',
  passwordMinLength: 'პაროლის სახელი უნდა შედგებოდეს მინ 3 სიმბოლოსგან',
  repeatPasswordRequired: 'პაროლის გამეორება აუცილებელია',
  passwordsMatch: 'პაროლები უნდა ემთხვეოდეს',

  // Api Errors
  usernameTaken: 'მომხმარებლის სახელი უკვე გამოყენებულია',
  emailTaken: 'ელ ფოსტა უკვე გამოყენებულია',
  usernameNoUser: 'მომხარებელი მსგავსი სახელით ვერ მოიძებნა',
  emailNoUser: 'მომხმარებელი მსგავსი ელ ფოსტით ვერ მოიძებნა',
  passwordInvalid: 'პაროლი არასწორია',
};
